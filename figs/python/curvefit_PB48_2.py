...
#fit PB1948
popt_l, pcov_l = opt.curve_fit(larrimore, PB1948[:, 0], PB1948[:, 1])
popt_g, pcov_g = opt.curve_fit(gaussianfit, PB1948[:, 0], PB1948[:, 1])
popt_t, pcov_t = opt.curve_fit(nctfit, PB1948[:, 0], PB1948[:, 1])
...
#calculate r-squared
residual_l = PB1948[:, 1] - larrimore(PB1948[:, 0], popt_l[0], popt_l[1])
ss_res_l = np.sum(residual_l**2)
ss_tot_l = np.sum((PB1948[:, 1] - np.mean(PB1948[:, 1]))**2)
r_squared_l = 1 - (ss_res_l / ss_tot_l)

residual_g = PB1948[:, 1] - gaussianfit(PB1948[:, 0], popt_g[0], popt_g[1])
ss_res_g = np.sum(residual_g**2)
ss_tot_g = np.sum((PB1948[:, 1] - np.mean(PB1948[:, 1]))**2)
r_squared_g = 1 - (ss_res_g / ss_tot_g)

residual_t = PB1948[:, 1] - nctfit(PB1948[:, 0], popt_t[0], popt_t[1])
ss_res_t = np.sum(residual_t**2)
ss_tot_t = np.sum((PB1948[:, 1] - np.mean(PB1948[:, 1]))**2)
r_squared_t = 1 - (ss_res_t / ss_tot_t)

residual_c = PB1948[:, 1] - TKfit(PB1948[:, 0], popt_c[0])
ss_res_c = np.sum(residual_c**2)
ss_tot_c = np.sum((PB1948[:, 1] - np.mean(PB1948[:, 1]))**2)
...
axes[0].plot(xx, wp1, 'b', lineWidth='2', label=r'$F^{L}_w=\delta F_p^{\gamma} / (\delta F_p^{\gamma} + (1-F_p)^{\gamma})$')
axes[0].plot(xx, wp2, 'r', lineWidth='2', label=r'Gaussian model')
axes[0].plot(xx, wp3, color='0.5', lineWidth='2', label=r"$t$-model")
axes[0].plot(xx, wp4, color='m', lineWidth='2', label=r"$F^{TK}_w = F_p^{\gamma} / ( F_p^{\gamma} + (1-F_p)^{\gamma})^{(1/\gamma)}$")
axes[0].plot(PB1948[:, 0], PB1948[:, 1], 'ok', markersize=5, label=r'Experiment')
axes[0].set_title('Preston & Baratta (1948)')
axes[0].set_xlabel(r'Cumulative probabilities $F_p$')
axes[0].set_ylabel(r'Cumulative weights $F_w$')
...
axes[1].plot(xx, wp1, 'b', lineWidth='2', label=r'$F^{L}_w=\delta F_p^{\gamma} / (\delta F_p^{\gamma} + (1-F_p)^{\gamma})$')
...
axes[1].set_xlabel(r'Cumulative probabilities $F_p$')
axes[1].set_ylabel(r'Cumulative weights $F_w$')
...
plt.savefig("./../curvefit_PB48_2.pdf", bbox_inches='tight')
plt.show()