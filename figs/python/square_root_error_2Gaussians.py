...
axes[0].set_title('Gaussian PDF and estimated PDF', fontsize='x-large')
axes[0].legend(loc='upper left', fontsize='x-large')
axes[0].set_yticks(np.arange(0, 0.6, step=0.1))

#plot CDFs
axes[1].plot(DO_CDF, DO_CDF,'r', label=r'$F_p(F_p)$', linewidth=2)
axes[1].axvline(x=0.5, LineStyle='--')
axes[1].plot(DO_CDF, DM_CDF,'b', label=r'$F_w(F_p)$', linewidth=2)
...
axes[1].set_title('inverse-S from estimation', fontsize='x-large')
axes[1].legend(loc='upper left', fontsize='x-large')
axes[1].set_yticks(np.arange(0, 1.1, step=0.2))

plt.savefig("./../square_root_error_1Gaussian.pdf", bbox_inches='tight')
plt.show()