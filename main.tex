...
Since \person{Pascal} and \person{Fermat} in their famous exchange of letters and later Huygens laid the foundations of probability theory in the 17th century as a tool to analyse gambles, expected value theory (EVT) formalised rational choice as an optimisation problem, where an expectation value of some quantity, \eg the gamble payoff, was thought to be \textit{the} suitable optimand.
%
Lack of predictive power and misalignment with empirical data led to an abundance of paradoxes and puzzles and to the development of ever new decision theories in response. However, newly developed theories only altered the quantities contained in the optimand, \ie probabilities and absolute monetary changes. In doing so ever more degrees of freedom have been added, while leaving the expectation value as the optimand intact. For instance, in expected utility theory (EUT) a new degree of freedom is introduced with the (possibly nonlinear) transformation of the absolute monetary changes. The axiomatisation of EUT by \cite{vonNeumannMorgenstern1944} and \cite{Savage1954} lend further inept credence to expectation values as optimands and EUT as a normative decision theory. As a reaction to violations of the cherished EUT axioms, \textcite{KahnemanTversky1979} proposed first prospect theory (PT) and later \textcite{TverskyKahneman1992} cumulative prospect theory (CPT) as descriptive decision theories, which allow nonlinear transformations of positive and negative absolute payoffs (value function) and the involved probabilities. The latter transformation is known as probability weighting and the focus of this paper. Following the consensus interpretation, probability weighting is a systematic maladaptive psychological effect of the overestimation of rare events and underestimation of common events.\footnote{An early example of this interpretation can be found in \textcite[Fig. 1, p. 188]{PrestonBaratta1948}.}

\subsubsection{Critiques of CPT} \seclabel{CPTCritiques}
Several studies question the validity of CPT and more specifically probability weighting. Some critiques concern the origin of the additional degrees of freedom.
% Origin of degrees of freedom 
Among others \textcite{StewartETAL2015} find no stable mappings from objective probabilities to subjective equivalents in individual DMs. They find that the specific mappings depend on the choice environment and are influenced to a large extent by the sampling method.
...
Research under the heading of the \textit{description-experience gap} further scrutinised the dependence on the sampling method. The description-experience gap signifies the common finding of stronger overweighting of rare events of subjects that learn model parameters (\eg probabilities) from summary descriptions (decisions-from-description) compared to subjects who estimate model parameters from experiential sampling over time and learn through sequential feedback after their individual choices (decisions-from-experience) \textcite{HertwigETAL2004}. Experience-based experiments even commonly find underweighting of rare events \parencite{UngemachETAL2009}.\footnote{It was realised that formerly dominant description-based experiments suffer from lessened external validity. Real-world decisions rarely provide clearly defined probabilities, see also \secref{Reasons_for}. Experience and learning are more important. Therefore, roughly since the year 2000, experience-based experiments prevail or studies contrast sampling modes like \textcite{HertwigETAL2004,HertwigErev2009,ErevETAL2010}. Furthermore, in animal studies only the decisions-from-experience sampling paradigm is feasible.}
...
Other studies emphasise statistical explanations rather than psychological explanations of probability weighting. For instance, \textcite{Martins2006} explains probability weighting as a heuristic that approximates an evolutionary optimal Bayesian solution to the inference problem of the DM. Similar to our results but in a Bayesian setting, \textcite{SeoETAL2019} find that optimal beliefs of the DM deviate from the true prior in a sequential learning problem. Thus it is optimal to overweight small priors and underweight large priors.
% Still PW after controlling sampling 
However, \textcite{FoxHadar2006,UngemachETAL2009} report that even after eliminating the sampling errors some (under)weighting persists, which suggests that some psychological effects exist beyond the statistical effects.

\subsubsection{Our contribution to the literature} \seclabel{OurContribution}
% is part of our critique and should go to the end (or the preview at the beginning).
Here we offer a statistical explanation of probability weighting that quite generally applies to any situation where differences between models of uncertainty arise.
%
As a result of the model difference the proposed mechanism is capable of generating arbitrary mismatches between objective probabilities in a normative model (used by a DO) and decision weights (used by a DM) and thus every possible shape of the probability weighting curve.
%
An inverse-S curve emerges in the case where the DM cautiously incorporates more uncertainty into his model than a DO does in his model (see \fref{mapping_pdfs} \& \fref{mapping_cdfs}). Our statistical explanation suggests that this behaviour is an adaptive strategy in decision making under uncertainty.
%
...
% 
Furthermore, this paper helps to showcase how psychological explanations might mislead, as they do not make clear testable predictions.
%
In contrast to other studies on the subject, we assume the DM to be aware of and to correct his estimate for his sampling error, whereas existing studies stop at the identification of the sampling error and thus believe the DM to use an underweighted and therefore biased estimate of rare events.

\subsubsection{Then all the other critiques (pick an order that makes sense)}
\textcite{PriceJones2020}
...
The research programme of ergodicity economics started when an alternative behavioural model was proposed in \textcite{Peters2011a,Peters2011b} and more recently in \textcite{Peters2019b},

which identifies that the relevant optimand for individual decision makers is based on the large-time limit


on the large-time limit of the growth rate of wealth

ato one containing an inappropriate ensemble average.

The deviation from the developmental path of most other decision theories went 

Although the statistical explanation we offer is strongly motivated by the ergodicity economics research programme, the particular strength is that it does not rely on any specific behavioural model (neither EVT nor EUT nor CPT nor growth-optimal preferences as in EE).

Once we discriminate between two uncertainty models 

The phenomenon labelled probability weighting is a result of a generic inference problem the DM is confronted with, much like in the experience-based setting.
