...
\ei
We express a probability distribution using a probability density function, $p(x)$, and a weight distribution using a weight density function, $w(x)$. Probabilities and weights for specific events are obtained as integrals of the corresponding density functions over the relevant range of $x$.\footnote{In the literature, weights are not always normalised, but for simplicity we will work with normalised weights. Mathematically, then, $w(x)$ is a proper probability density function, although for clarity we avoid referring to it as such. Our results are unaffected because normalising just means dividing by a constant (the sum or integral of the unnormalised weights).}
...
\caption{\textbf{Empirical phenomenon of probability weighting.} Weights \vs probabilities as reported by \textcite{PrestonBaratta1948}, the first to document probability weighting empirically. (They refer to probabilities as ``mathematical probabilities,'' weights as ``psychological probabilities,'' and consider events of non-cumulative type.) Low probabilities (left) are exceeded by their corresponding weights and \vv for high probabilities (right). Fitting a curve to such data would produce something resembling an inverse-S.}
\flabel{PB1948}
\end{figure}
%\begin{figure}[!htb]
%\centering
%\includegraphics[width=0.5\textwidth]{./figs/TK1992.pdf}
%\caption{\textbf{Empirical phenomenon of probability weighting.} Cumulative weights $F_w$ (used by decision makers) versus cumulative probabilities $F_p$ (used by disinterested observers), as reported by \textcite[p.~310, Fig. 1, relabelled axes]{TverskyKahneman1992}. The figure is to be read as follows: pick a point along the horizontal axis (the cumulative probability $F_p$ used by a DO) and look up the corresponding value on the vertical axis of the dotted inverse-S curve (the cumulative weight $F_w$ used by a DM). Low cumulative probabilities (left) are exceeded by their corresponding cumulative weights, and for high cumulative probabilities it's the other way around. It's the inverse-S shape of the curve that indicates this qualitative relationship.}
%\flabel{TK1992}
%\end{figure}
...
%An additional apparent distortion of probabilities has been reported as early as in \textcite[Fig. 1, p. 188]{PrestonBaratta1948}, who introduced \textquote{psychological probabilities} as a function of mathematical probabilities to reconcile experimental data with EVT.
...
%\parencite{PrestonBaratta1948,KahnemanTversky1979,TverskyKahneman1992}.
...
%The mechanism we propose can generate mismatches between probabilities and weights which produce a variety of probability weighting curves, including the standard inverse-S curve (see \fref{mapping_cdfs}).
%The inverse S is predicted whenever a DM cautiously incorporates more uncertainty into her model than a DO, which we imagine to be a commonly encountered scenario in experiments.
...
%\footnote{Note that the argument of $F^{TK}$ in \eref{correspondence} and $F^L$ in \eref{LattimoreFunction} is the value of a CDF, not the value of $x$.} 
...
%This corresponds to a nonlinear distortion of the horizontal axis.
%Because of the nonlinear distortion of the horizontal axis, they are shifted to different locations horizontally.
}
\flabel{mapping_pdfs}
\end{figure}

\AA{We no longer use the acronym PDF, since it does refer efficiently to the weight density function. We simply say density function now. Some axis labels need to be changed as a result.}

If the DM uses a greater scale in her model, then weight densities are higher than probability densities where the densities are low, and (because of normalisation) lower than probability densities where the densities are high. We can express this by plotting, for any value of $x$, the weight \vs probability densities observed at $x$, as shown in right panel of \fref{mapping_pdfs}.
...
\end{enumerate}
Of course, the DM could even assume a weight distribution whose shape differs from that of the DO's probability distribution.
The inverse-S arises whenever a DM assumes a greater scale for a unimodal distribution.
To illustrate the generality of the procedure, we carry it in \fref{Student-t} out for Student's (power-law tailed) $t$-distributions (hereafter $t$-distributions), where the DO and DM use different shape and location parameters.\footnote{
...
\AA{Someone has left a ``????'' in the footnote below. What is this?}
...
%How exactly it is linked to physical reality is not completely clear.
...
Considering the noise in the data in \fref{PB1948}, one might speculate that the inverse-S function of \textcite{TverskyKahneman1992}, $F^{TK}_w(F_p)$ in \eref{correspondence}, would fit the data no better than the functions arising from our mechanistic model. This is particularly evident in the bottom panels of \fref{Gauss_scale_location_both_KT}, which show that a Gaussian, $w(x)$, whose scale and location differ from those of $p(x)$, reproduces the functional shape of $F^{TK}_w(F_p)$ fitted to the data in \parencite{TverskyKahneman1992}.

\AA{Hmm, bit of a mess with different data sources now!}
...
%, a behavioural economist, legal scholar, and former United States Administrator of the Office of Information and Regulatory Affairs, 
people's concern about a potential coronavirus outbreak in the United States was attributable to an extreme case of probability weighting: people were neglecting the fact that such an event had a low probability. When it was published, many disagreed: it seemed reasonable to them to take precautions; the author himself, they said, might have underestimated both the severity and likelihood of a pandemic. In effect, they asked the \textcite{Cohen1979a} question: ``Whose is the fallacy?''

As we write this, in November 2020, more than 250,000 Americans have died of the virus.
...
A starting point of other people's judgments being systematically flawed can lead us to disregard the possibility that we ourselves may be wrong.
...
\elabel{rel_error}
\ee
where $\frac{\err{p(x)}}{p(x)}$ is the relative error, and the denominator of \eref{rel_error} is a normalisation constant. If the relative error is larger for small densities than for large densities, then small densities are enhanced more (the summand $\frac{\err{p(x)}}{p(x)}$ in the numerator is greater) than large densities. The normalisation constant scales down all densities equally and, where the enhancement was greater, $w(x)$ ends up above $p(x)$. Where it was lower, $w(x)$ ends up below $p(x)$. So, if the relative error is larger for small densities, an inverse-S curve emerges.
