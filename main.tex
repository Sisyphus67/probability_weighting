...
[14:28, 09/04/2020] Alex: es. I believe the Poisson setup looks like this. Count n(T) after time T is Poisson distributed with intensity parameter pT such that E[n(T)]=pT and stddev[n(T)] = sqrt(pT). If we make one observation per time unity, we can measure phat = n(T)/T as an estimate of a probability.
[14:29, 09/04/2020] Alex: E[phat] = p, i.e. the estimator is unbiased. Standard error in phat is sqrt(phat/T) and standard relative error is sqrt(1/phat*T).
[14:32, 09/04/2020] Alex: That all works and we only need to insert the 1/T into the existing text. Somehow it is implicit that we do not use this method to estimate probabilities of very common discrete events, where the correspondence between the continuous and discrete models breaks down.
[14:36, 09/04/2020] Alex: *time unit
[14:39, 09/04/2020] Ole Peters: yes. One observation per time unit, and choose the time unit so that events are rare (much fewer than 1 per time unit). Then phat is an estimate of the probability of seeing an even in one observation.
[14:40, 09/04/2020] Ole Peters: *event
[14:41, 09/04/2020] Ole Peters: To translate between CDF and PDF one differentiates/integrates along another dimension.
[14:42, 09/04/2020] Ole Peters: Let's say phat is the probability estimate that a<x<b. Then the density estimate is phat/(b-a).
[14:47, 09/04/2020] Ole Peters: ...so one can mess with the Delta x interval (b-a), instead of the time unit, to make sure one doesn't have too many observations.
[14:48, 09/04/2020] Alex: Yes. I think this setup is ok. We lose a little generality because events must be rare, but we have a simpler uncertainty term. Somehow I prefer the repeated Bernoulli setup conceptually, but the extra (1-p) term doesn't seem worth it.
[14:49, 09/04/2020] Alex: If so, we are back to the original with the 1/T added plus some explanatory text along the above lines. Agreed?

\subsubsection*{Estimation errors for probabilities}
Imagine the DO and DM have the same interpretation of the word ``probability.'' Say they agree that they mean the relative frequency of an event in an infinitely long time series of observations. Of course, real time series have finite length, so probabilities defined this way are never known with certainty. They can only be estimated, by counting. If an event occurs in $n$ out of $T$ observations, then our best estimate for its probability is $\frac{n}{T}$

As the probability of an event gets smaller, so the number of times we see it in a finite time series gets smaller. Moreover, fluctuations in this number -- and, therefore, the uncertainty in our estimate of the probability -- become relatively larger as an event becomes rarer. Let's take an extreme example of an event which occurs in 0.1\% observations asymptotically, of which we have a time series of 100 observations. Almost all such series (around 99.5\% of them) will contain 0 or 1 events, meaning that we will estimate the probability as either 0 or 1\%. In other words, we estimate that the event is either impossible or that it occurs ten times more frequently than it really would in a long series. If, however, the event occurs 50\% of the time asymptotically, then a probability estimate from 100 observations would likely (in around 95\% of series) lie in the range 40--60\%.

Why is this important? If the relative uncertainty in a probability estimate becomes a larger as an event becomes rarer, then a DM who must estimate probabilities from observations is well advised to account for this in his decision-making. Specifically, he should acknowledge that, due to his lack of information, {\it prima facie} rare events may be rather more common than his data imply, while common events, being revealed more often, are more easily quantified. In such circumstances, caution may dictate that he assigns higher probabilities to rare events than he has estimated, in line with probability weighting.

We can build a simple model to see if it matches the stylised facts of probability weighting, specifically the inverse-S curve. In the simplest case (and we rarely consider anything more complicated), we model the arrival of events as a Poisson process, where the standard error in the count of an event famously goes as $\sqrt{n_i}$. The standard error in the probability of an event is therefore $\frac{\sqrt{n_i}}{T}$, and the relative error is $\frac{1}{\sqrt{n_i}}$. Low probabilities therefore come with larger relative errors, see \tref{errors}. We note that behaviourally, it will make little difference whether a DM assigns a 0.49 probability to an event or a 0.51 probability. It will make a large difference, however, whether a DM assigns a 0.0001 probability or a 0 probability.
