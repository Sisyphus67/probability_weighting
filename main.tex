...
%\ee
\be
w(x)\delta x = \hat{p}(x)\delta x + 2\sqrt{\frac{\hat{p}(x)}{T}}\delta x
= \left(\hat{p}(x) + 2\sqrt{\frac{\hat{p}(x)}{T}}\right)\delta x
\ee
\OP{The uncertainty is only in the count $n$, whereas the interval width $\delta x$ is known and can be treated as a constant factor. If the uncertainty in $n$ is $\sqrt{n}$, then the uncertainty in $\delta x n$ is $\delta x \sqrt{n}$.}\\
\OP{We don't need to assume 2 standard errors, one is also ok -- I think I did it to make the figure look clearer, but we can do that by tuning $T$ now.}\\
\OP{We're overloading notation, with $\hat{p}$ being both a probability and a density...}\\
...
\be
w(x)=\frac{\hat{p}(x)+2\sqrt{\hat{p}(x)/T}}{\int_{-\infty}^{\infty}[\hat{p}(s)+2\sqrt{\hat{p}(s)/T}] ds},
\elabel{Poisson}
\ee
where the denominator ensures normalization.\\ 
\OP{check code for factors of 2 (there was one missing in the numerator).}\\
Note the role of $T$ here: as $T$ grows large, \ie as the DM collects more information, the cautionary correction term vanishes, and both $w(x)$ and $\hat{p}(x)$ converge to the asymptotic density, $p(x)$. With perfect information, a DM does not need to adjust decisions to account for uncertainty.\\
\OP{We might want to do this numerically to make sure we understand the role of $T$, \ie simulate time series, estimate $p$ and its error \etc, compare resulting $w$ to $p$.}
