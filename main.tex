...
{\it Probability weighting} is a concept that originated in prospect theory \citep{KahnemanTversky1979,TverskyKahneman1992}. It is one way to conceptualise a pattern in human behaviour of caution with respect to formal models.
...
Assuming that ``no theory of choice can be both normatively adequate and descriptively accurate'' \citep[p.~251]{TverskyKahneman1986}, they classified prospect theory as descriptive rather than normative, \ie as relating to actual rather than optimal behaviour.
Put simply, prospect theory ia an attempt to model systematic errors in human decision making, arising (in part) from inappropriate psychological adjustments of known probabilities.

We caution against this interpretation of probability weighting as a cognitive bias leading to suboptimal decisions.
...
We are by no means the first to raise this question. Commenting on another so-called cognitive bias regarding probabilities (the representative fallacy), \citet{Cohen1979a} asked: ``Whose is the fallacy?''
...
\citet{Gigerenzer2018} pointed out that simply avoiding probabilities altogether often resolves apparent disagreements between DO and DM.
This can sometimes be achieved by using what he calls natural frequencies, such as ``10 out of 100 people.''
The discussion in \citep{Gigerenzer2018} focuses on the relative ease with which humans reason about and manipulate integers.
To this we add that the statement ``10 out of 100 people have this disease'' conveys more, and more precise, information than ``the probability of having this disease is 0.1.''
Specifically, it tells us that a sample of people has been observed and the size of the sample.
...
\caption{{\bf Model fitting to experimental data from \citet{TverskyKahneman1992} (left) and \citet{TverskyFox1995} (right).}
