...
Does our analysis, culminating in \eref{weight_density}, reproduce the stylised facts of probability weighting, in particular the inverse S curve? 
%Let's re-write \eref{weight_density} as
%\bea
%w(x)&=&\hat{p}(x)\left(\frac{1+\sqrt{\frac{1}{\hat{p}(x) T \delta x}}}{1+\mathrm{E}\left[\sqrt{\frac{1}{\hat{p}(x) T \delta x}}\right]}\right)\\
%\quad&\text{where}& \mathrm{E}\left[\sqrt{\frac{1}{\hat{p}(x) T \delta x}}\right] \equiv \int_{-\infty}^{\infty} \hat{p}(s)\sqrt{\frac{1}{\hat{p}(s)T \delta x}}ds.
%\eea
We check in two ways. First, analytically, by applying the DM's cautionary correction in \eref{weight_density} to reference probability density functions. Second, by simulating the DM compiling counts of outcomes drawn from reference distributions, from which he estimates probability densities and their uncertainties. In both cases, we treat the DO as using the reference distribution to make his predictions of the DM's behaviour.
...
\AA{Placeholder for simulations}
