...
The foundations of formal probability theory were laid in the 1650s as a tool to analyse gambles. These early developments led to expected value theory (EVT), which formalises choosing rationally among gambles as optimising the expectation value of the gamble payoff.
...
%EUT
While successful in some circumstances, EVT led to paradoxes and puzzles in others. The first proposed remedy to these paradoxes was expected utility theory (EUT) \parencite{Bernoulli1738,Laplace1814}, which distorts monetary amounts to reflect people's apparent psychological risk aversion. The structure of an expectation value (a weighted sum) as the optimand was kept intact. The axiomatisation of EUT by \textcite{vonNeumannMorgenstern1944} and \textcite{Savage1954} helped establish EUT as a normative decision theory.

%CPT
But despite its flexibility, EUT also failed to fit some observations, and (cumulative) prospect theory (CPT) \parencite{KahnemanTversky1979,TverskyKahneman1992} was proposed as a descriptive decision theory. CPT adds distortions of probabilities to EUT's distortions of monetary amounts. The distortion of probabilities is known as probability weighting and is the focus of this paper. Following the consensus interpretation, probability weighting is a systematic maladaptive psychological effect of the overestimation of rare events and underestimation of common events.\footnote{An early example of this interpretation can be found in \textcite[Fig. 1, p. 188]{PrestonBaratta1948}.}
...
As research continued, critiques of CPT, and probability weighting emerged. With both monetary amounts and probabilities replaced by freely chosen nonlinear functions, CPT is extremely flexible and in danger of overfitting: it may fit past data well, but parameters may be unstable when fitted again to new data.
Indeed, among others \textcite{StewartETAL2015} find no stable mappings from objective probabilities to subjective equivalents in individual DMs. They find that the specific mappings depend on the choice environment and are influenced to a large extent by the sampling method.
...
A key confounder -- a variable that hadn't been controlled for initially -- turned out to be the experimental design. Research under the heading of the \textit{description-experience gap} revealed that it matters how DMs are informed of probabilities in a decision problem. In \textit{decisions-from-description} DMs are told the probabilities by DOs. In \textit{decisions-from-experience} DMs infer model parameters from experience over time. Since there are different possible physical interpretations of probabilities, see also \secref{Reasons_for}, decisions from description lack potentially relevant semantic information, whereas decisions from experience are clearer in this respect but the DM is faced with matters of statistical significance. \textcite{HertwigETAL2004} find different (in their case stronger) overweighting of rare events in decisions-from-description than in decisions-from-experience. These studies call into question the reproducibility of probability weighting. Some experience-based experiments have even found underweighting of rare events \parencite{UngemachETAL2009}.\footnote{Roughly since the year 2000, experience-based experiments or studies that contrast sampling modes prevail \parencite{HertwigETAL2004,HertwigErev2009,ErevETAL2010}. In animal studies only the decisions-from-experience sampling paradigm is feasible.}
...
Decision-from-experience studies emphasise statistical explanations rather than psychological explanations of probability weighting. For instance, \textcite{Martins2006} explains probability weighting as a heuristic that approximates an evolutionary optimal Bayesian solution to the inference problem of the DM. In line with our analysis, though in a Bayesian setting, \textcite{SeoETAL2019} find that optimal beliefs of the DM deviate from the true prior in a sequential learning problem. Thus it is optimal to overweight small prior probabilities and underweight large ones. \textcite{FoxHadar2006,UngemachETAL2009}, on the other hand, report that probability weighting persists even after controlling for such effects, suggesting that a psychological component may still exist.
\OP{1. Need a summary sentence here: In short, the status of PW is unclear; inverse S is to be expected in most setups but not in all. 2. Mark was thinking to put PriceJones2020 here. Let's discuss; it would be nice to connect there.}
...
In our own work, ergodicity economics, the DM's experience of outcomes along a timeline is fundamental \parencite{Peters2019}. In this paper we thus ask what a DM can reasonably infer about probabilities from the available information, \eg a statement by a DO with no corroborating evidence or a finite sample of observed outcomes. In decision theory as it pertains to individuals, ergodicity economics rejects EVT because there is no physical reason for expectation values to reflect what happens to an individual in the long run. Instead ergodicity economics uses as its null-model the optimization of appropriate growth rates evaluated in the long-time limit \parencite{Peters2011a,Peters2011b,Peters2019b}. For clarity: this is in contrast to conventional decision theories, including CPT, which maintain expectation values (suitably distorted) as their optimands. As a consequence of this conceptual difference, ergodicity economics finds that growth-optimal behaviour, defined properly along a single timeline, predicts correctly a number of deviations from standard EUT \parencite{AdamouETAL2019,MederETAL2019}. Our mechanism explains probability weighting as arising from the distinction between the models of uncertainty held by the omniscient DO (who defines the setup) and held by the DM (who is only given partial information). %Importantly, this is independent from a particular behavioural model -- neither EVT, EUT, CPT nor growth-optimal preferences. The phenomenon labelled probability weighting is a result of a generic inference problem, in which the DM is simply confronted with a different risk than the DO.
...
%
An inverse-S curve is expected in typical situations, namely whenever the DM cautiously incorporates more uncertainty into his model than a DO does in his. 
...
%
Specifically, we point out that an inverse S is consistent with the DM cautiously adjusting a frequentist probability estimate for sampling errors, whereas existing studies stop at the identification of the sampling error, and thus believe the DM uses an underweighted and biased estimate of rare events.
