...
%     backrefpage  = {cit. on p.}, 			% for single page number
%     backrefpage  = {}, 								% for single page number
%     backrefpages = {cit. on pp.} 			% for multiple page numbers
...
\ee
\ei
In \fref{TK1992} we reproduce the first such visual summary from \textcite[p.~310]{TverskyKahneman1992}.
\begin{figure}[!htb]
\centering
...
\ee
%
suggested by \textcite{LattimoreBakerWitte1992} to parametrically describe probability weighting (also used by \textcite{tversky1995risk} and \textcite{Prelec1998}). The reason for fitting \eref{LattimoreFunction} is to ensure a fair comparison: the Gaussian and $t$ models are characterised by two parameters, whereas \eref{correspondence} only has one free parameter. \Eref{LattimoreFunction} has two parameters.
