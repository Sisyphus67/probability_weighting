...
Let's assume that both the DO and the DM mean by ``probability'' the relative frequency of an event in an infinitely long time series of observations. Of course, real time series have finite length, so probabilities defined this way are model parameters. But they can be estimated from a real time series, by counting. If an event $i$ occurs $n_i$ times in a time series of length $T$, then our best estimate for its relative frequency is $n_i/T$. 
