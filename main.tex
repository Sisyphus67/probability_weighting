...
\ee
%
where $\Phi$ is the CDF of the standard normal distribution. In the Student's-t model we fit the location and scale parameters $\mu$ and $\nu$ in the CDF $F_w\left(x\right)$ of a non-central Student's t-distributed random variable, assuming that the variable $x$ is given by the Student's t inverse CDF with $\mu=0$ and $\nu=1$ of $F_p$.
