...
\AA{We should show a plot of this inverse-S curve here, since we no longer show it in \fref{PB1948}. The above discussion is hard to follow without a visual reference. Option 1: fit to the TK1992 data, which we use (without introduction) in \fref{Gauss_scale_location_both_KT}. Option 2: fit to PB1948 data and change \fref{Gauss_scale_location_both_KT} to use these data too.}
...
\AA{Suggest axis labels: x, density (left) and p, density (right).}
}
\flabel{mapping_pdfs}
\end{figure}
...
To illustrate the generality of the procedure, we carry it in \fref{Student-t} out using Student's $t$-distribution (hereafter $t$-distribution), with the DO and DM assuming different shape and location parameters.\footnote{
...
Probability weighting is usually interpreted as a cognitive bias that leads to errors of judgement and poor decisions by DMs.\footnote{Indeed, its originators presented it as such. Devising a gambling game played by human subjects, \textcite{PrestonBaratta1948} argued that ``the payment of amounts in excess of the mathematical expectation for the privilege of participation is peculiarly irrational'' and claimed that ``the failure of the price to correspond with the mathematical expectation is due to peculiarities of the player's notion of the meaning of a given probability.''}
%that time-average changes in wealth due to gambling are to be modelled as identical to their expectation values, \ie that wealth changes under gambling is to be modelled as ergodic. This leads them to conclude that ``????the payment of amounts in excess of the mathematical expectation [...] is peculiarly irrational.'' We note that this declares irrational the purchase of any typical insurance product.
...
\AA{Suggest axis labels x, density (top left) and p, density (top right).}
}
