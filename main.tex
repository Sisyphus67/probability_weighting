...
Labelling \textit{probability weighting} as a cognitive bias suggests that it constitutes systematic errors of judgement.
%comes with a connotation of erroneous behaviour,
...
%
%Thus behavioural economics gave rise to a rhetoric of irrationality, which invites views of the necessity to nudge people's \textit{irrational} behaviour in the \textit{right} direction \citep{Lopes1991,Gigerenzer2018}. 
We caution against this interpretation. At least we should keep in mind that it is unclear who suffers from the bias: experimenter or test subject? 
%We are by no means the first to raise this question. Commenting on another so-called cognitive bias regarding probabilities (the representative fallacy), \citet{Cohen1979a} put it thus: ``Whose is the fallacy?''
%\OP{Question: has anyone raised this question specifically about probability weighting? Otherwise, the point is a bit forced (though the paper looks great!): in any disagreement about anything, the question is: who is wrong?}
...
%\OP{We don't explain what ``cognitive illusions'' are, and it's probably too much of a distraction to do so.}
% 
\citet{Gigerenzer2018} pointed out that simply avoiding probabilities altogether often resolves apparent disagreements between DO and DM. This can sometimes be achieved by using what he calls natural frequencies, such as ``10 out of 100 people.'' The discussion in \citep{Gigerenzer2018} focuses on the relative ease with which we reason about and manipulate integers. To this we add that the statement ``10 out of 100 people have this disease'' conveys far more, and more precise, information than ``the probability of having this disease is 0.1.''
...
%\OP{Also ecological rationality seems like a big can of worms that could derail the manuscript}
...
%, where we provide an analytical solution for the conceptual insight about ecological rational behaviour.
