\title[\insertlogo\\
\vspace{.5em}
% Ergodicity Economics\\
%\vspace{.2cm}
%\underline{Part 2} Applications.\\
\lmlblue{Mark Kirstein}]{\textbf{
What are we weighting for?
}}
\subtitle{A mechanistic model for probability weighting}

% Authors with Email addresses as footnotes
% \author{Ole Peters \and Alexander Adamou \and Yonatan Berman \and Mark Kirstein}

\author[M Kirstein]{
Ole Peters \and Alexander Adamou \and Yonatan Berman \and Mark Kirstein\\
\vspace{.5em}
{\small
\href{mailto:m.kirstein@lml.org.uk}{\Letter ~ \texttt{m.kirstein{\MVAt}lml.org.uk}} \\
	\href{https://twitter.com/nonergodicMark}{\includegraphics[height=1.4em,valign=c]
	{../../img/logos/Twitter_Logo_Blue} \MVAt \texttt{nonergodicMark}}
}
}

\date[17 June 2020]{
D-TEA 2020, 16 June 2020\\
\vspace{.5cm}
% Logo MPIMIS
\href{https://www.mis.mpg.de/jjost/research/economics.html}{
\includegraphics[height=1.1cm]{../../img/logos/MPIMIS_en.png}
% \includegraphics[height=1.0cm]{../../img/logos/MPIMIS_Pyramide.png}
}
\hfill
% Logo Uni LE
\href{https://www.math.uni-leipzig.de/cms/de/forschung/abteilungen/wima/}{
\includegraphics[height=1.1cm]{../../img/logos/Uni_Leipzig_MathemInstitut.jpg}
% \includegraphics[height=1.0cm]{../../img/logos/UniLE_logo_neu}
}
\hfill
% Logo LML
\href{http://lml.org.uk}{
% \includegraphics[height=1.cm]{../../img/logos/LML_logo_only.jpg}
\includegraphics[height=1.1cm]{../../img/logos/LML_LOGO_whiteBG.jpg}
}
}
