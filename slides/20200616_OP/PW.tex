...
XXX label on left figure: change to ``Ergodicity economics'' to mirror K\&T?
\begin{enumerate}
...
XXX produce figure with Gaussian and Student-t with heavy tails to illustrate shape difference
\end{frame}
...
To avoid surprises, let's say DMs add estimation uncertainty $\err{p(x)}$ to every estimated probability, then normalize, s.t.
...
...visually similar to function chosen by Kahneman and Tversky.

XXXNot sure we need much more. I'd just have one figure that gives a nice inverse S, for a Gaussian, say, based on estimation error.
...
%   \item relative probability estimation errors are greater for rarer events
  ...
  \item Interactive code at \url{https://bit.ly/lml-pw-code}
  
  XXX researchers.one link is better: an opportunity to advertise R1 and explain it's open for public review.
  
  XXX code link (from manuscript) goes to wrong place -- link to binder for interactive notebook.
