...
To avoid surprises, let's say DMs \blue{add estimation uncertainty $\err{p(x)}$} to every estimated probability, then normalize, s.t.
...
where DM's greater scale is $(\alpha\sigma)^2$
% \ldots visually similar to function chosen by Kahneman and Tversky.
\end{frame}

\begin{frame}{Estimating probabilities for two Gaussians}
